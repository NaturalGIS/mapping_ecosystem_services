...
##lu_geom_name="$(ogrinfo -so $datasource $land_use_layer | grep -w 'Geometry Column =' | sed 's/Geometry Column = //g')"
sa_geom_name="$(ogrinfo -so $datasource $land_use_layer | grep -w 'Geometry Column =' | sed 's/Geometry Column = //g')"
...
echo "Importing the study area map..."
ogr2ogr -q --config PG_USE_COPY YES -f "PostgreSQL" "PG:host=localhost user=$username dbname=$dbname password=$password" -lco SPATIAL_INDEX=$spatialindex -lco GEOMETRY_NAME=geom -lco FID=gid -nln $schemaname.study_area -nlt MULTIPOLYGON $datasource $study_area_layer -overwrite -lco OVERWRITE=YES -t_srs EPSG:$crs

echo "Buffering the study area map..."
ogr2ogr -q --config PG_USE_COPY YES -f "PostgreSQL" "PG:host=localhost user=$username dbname=$dbname password=$password" -lco SPATIAL_INDEX=$spatialindex -lco GEOMETRY_NAME=geom -lco FID=gid -nln $schemaname.study_area_buffered -nlt MULTIPOLYGON $datasource -dialect SQLITE -sql "SELECT 1 AS fid, ST_Union(ST_Buffer($sa_geom_name,2*$distance)) AS geom FROM $study_area_layer" -overwrite -lco OVERWRITE=YES -t_srs EPSG:$crs
...
ogr2ogr -q --config PG_USE_COPY YES -f "PostgreSQL" "PG:host=localhost user=$username dbname=$dbname password=$password" -lco SPATIAL_INDEX=$spatialindex -lco GEOMETRY_NAME=geom -lco FID=gid -nln $schemaname.land_use_original -nlt MULTIPOLYGON $datasource $land_use_layer -overwrite -lco OVERWRITE=YES -t_srs EPSG:$crs
